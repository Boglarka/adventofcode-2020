...
$input = "LLLLLLLL.LLLL.LLLLL.LLLLLLLLLLLLLL.LLLLLLLLLLLLLL.LLLLL.LLLLLLLLLLLLLL.LLLLLLL.LLLLLL.LLLLLLLLLL.L
...
function check_adjacent($lines, $i, $j) {
  $cnt = 0;
  for($k = -1; $k < 2; $k++) {
    for($l = -1; $l < 2; $l++) {
      if (($k != 0 || $l != 0)
          && $i+$k >= 0 && $j+$l >= 0
          && $i+$k < sizeof($lines) && $j+$l < strlen($lines[0])
          && $lines[$i+$k][$j+$l] == '#') $cnt++;
    }
  }
  return $cnt;
  ...
  do {
    ...
    for($i = 0; $i < sizeof($lines); $i++) {
      for($j = 0; $j < strlen($lines[$i]); $j++) {
        $cnt = check_adjacent($old_lines, $i, $j);
        if ($lines[$i][$j] == 'L' && $cnt == 0) {
          $lines[$i][$j] = '#';
        } else if ($lines[$i][$j] == '#' && $cnt >= 4) {
          $lines[$i][$j] = 'L';
        }
      }
    }
...
function check_first($lines, $i, $j) {
  $cnt = 0;
  for($k = -1; $k < 2; $k++) {
    for($l = -1; $l < 2; $l++) {
      $row = $k;
      ...
      while (($k != 0 || $l != 0)
             && $i+$row >= 0 && $j+$col >= 0
             && $i+$row < sizeof($lines) && $j+$col < strlen($lines[0])) {
        if ($lines[$i+$row][$j+$col] != '.') {
          if ($lines[$i+$row][$j+$col] == '#') $cnt++;
          break;
        }
        $row += $k;
        $col += $l;
...
function day2($input) {
  ...
  do {
    ...
    for($i = 0; $i < sizeof($lines); $i++) {
      for($j = 0; $j < strlen($lines[$i]); $j++) {
        $cnt = check_first($old_lines, $i, $j);
        if ($lines[$i][$j] == 'L' && $cnt == 0) {
          $lines[$i][$j] = '#';
        } else if ($lines[$i][$j] == '#' && $cnt >= 5) {
          $lines[$i][$j] = 'L';
        }
      }
    }
