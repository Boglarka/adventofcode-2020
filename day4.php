...
function day2($input) {
  ...
  for($i = 0; $i < sizeof($lines); $i++) {
    ...
    foreach($keyvals as $keyval) {
      switch($keyval[0]) {
        case "byr":
          if ((int)$keyval[1] >= 1920 && (int)$keyval[1] <= 2002) ${$keyval[0]} = TRUE;
          break;
        case "iyr":
          if ((int)$keyval[1] >= 2010 && (int)$keyval[1] <= 2020) ${$keyval[0]} = TRUE;
          break;
        case "eyr":
          if ((int)$keyval[1] >= 2020 && (int)$keyval[1] <= 2030) ${$keyval[0]} = TRUE;
          break;
        case "hgt":
          if ($pos = strpos($keyval[1], 'in') !== FALSE && (int)$keyval[1] >= 59 && (int)$keyval[1] <= 76) {
            ${$keyval[0]} = TRUE;
          } else if ($pos = strpos($keyval[1], 'cm') !== FALSE && (int)$keyval[1] >= 150 && (int)$keyval[1] <= 193) {
            ${$keyval[0]} = TRUE;
          }
          break;
        case "hcl":
          if (preg_match('/^#[0-9a-f]{6}$/', $keyval[1])) ${$keyval[0]} = TRUE;
          break;
        case "ecl":
          if (array_search($keyval[1], ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth']) !== FALSE) ${$keyval[0]} = TRUE;
          break;
        case "pid":
          if (preg_match('/^[0-9]{9}$/', $keyval[1])) ${$keyval[0]} = TRUE;
          break;
      }
    }
    if ($byr && $iyr && $eyr && $hgt && $hcl && $ecl && $pid) {
