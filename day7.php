...
$input = "drab tan bags contain 4 clear gold bags.
...
function check($bags, $lines, $text) {
  for($i = 0; $i < sizeof($lines); $i++) {
    if (($pos = strpos($lines[$i], $text)) !== FALSE && $pos != 0) {
      preg_match('/^[a-z ]+ bag/', $lines[$i], $match);
      $newbags[] = $match[0];
      if (!in_array($match[0], $bags)) {
        $bags[] = $match[0];
      }
    }
  }
  for($i = 0; $i < sizeof($newbags); $i++) {
    $bags = check($bags, $lines, $newbags[$i]);
  }
  return $bags;
}
...
function day1($input) {
  $lines = explode("\n", $input);
  $res = 0;
  $bags = check([], $lines, "shiny gold");
  for($i = 0; $i < sizeof($lines); $i++) {
    for($j = 0; $j < sizeof($bags); $j++) {
      if (($pos = strpos($lines[$i], $bags[$j])) !== FALSE) {
        $res++;
        break;
      }
    }
...
vibrant plum bags contain 5 faded blue bags, 6 dotted black bags.
faded blue bags contain no other bags.
dotted black bags contain 2 almafa bags.
almafa bags contain no other bags.";*/
//1 + 2 + 1*3 + 1*4 + 2*5 + 2*6 = 32

function check2($res, $lines, $text, $count) {
  for($i = 0; $i < sizeof($lines); $i++) {
    if (($pos = strpos($lines[$i], $text)) !== FALSE && $pos == 0) {
      preg_match_all('/[0-9]+ [a-z ]+ bag/', $lines[$i], $matches);
      for($j = 0; $j < sizeof($matches[0]); $j++) {
        preg_match('/^[0-9]+/', $matches[0][$j], $num);
        preg_match('/[a-z ]+/', $matches[0][$j], $str);
        $str = trim($str[0]);
        $num = $num[0];
        $count[] = $num;
        $res = check2($res, $lines, $str, $count);
        $multi = 1;
        foreach ($count as $c) {
          $multi *= $c;
        }
