...
function day1($input) {
  ...
  foreach($lines as $line) {
    $subs = explode(" ", $line);
    $minmax = explode("-", $subs[0]);
    $letter = explode(":", $subs[1]);
    $count = preg_match_all("/$letter[0]/", $subs[2]);
...
function day2($input) {
  ...
  foreach($lines as $line) {
    ...
    $minmax = explode("-", $subs[0]);
    $letter = explode(":", $subs[1]);
    if (($subs[2][$minmax[0]-1] == $letter[0] && $subs[2][$minmax[1]-1] != $letter[0])
        || ($subs[2][$minmax[0]-1] != $letter[0] && $subs[2][$minmax[1]-1] == $letter[0])) {
      $res++;
    }
  }
